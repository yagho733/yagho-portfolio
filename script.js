...
// Letreiro final: um único loop via requestAnimationFrame para manter o mesmo comportamento em Safari/iPhone e desktop.
const contactTrack=qs('.contact-track');
if(contactTrack){
  contactTrack.style.setProperty('animation','none','important');
  if(reduce){
    contactTrack.style.setProperty('transform','translate3d(0,0,0)','important');
  }else{
    let marqueeOffset=0,marqueeLast=0,marqueeWidth=0;
    const measureMarquee=()=>{marqueeWidth=contactTrack.scrollWidth/2};
    const startMarquee=()=>{
      measureMarquee();
      if('ResizeObserver' in window)new ResizeObserver(measureMarquee).observe(contactTrack);
      else addEventListener('resize',measureMarquee,{passive:true});
      const runMarquee=now=>{
        if(!marqueeLast)marqueeLast=now;
        const dt=Math.min((now-marqueeLast)/1000,.05);
        marqueeLast=now;
        const speed=innerWidth<=720?52:36;
        if(marqueeWidth>0){
          marqueeOffset=(marqueeOffset+speed*dt)%marqueeWidth;
          contactTrack.style.setProperty('transform',`translate3d(${-marqueeOffset}px,0,0)`,'important');
        }
        requestAnimationFrame(runMarquee);
      };
      requestAnimationFrame(runMarquee);
    };
    if(document.fonts?.ready)document.fonts.ready.then(()=>requestAnimationFrame(startMarquee));
    else requestAnimationFrame(startMarquee);
  }
}
